...
\begin{equation}
\langle \alpha\left|\right.=(\left|\right.\alpha\rangle )^\dagger \leftarrow \text{hermetisch 
Adjungiert}
\end{equation}
   ...
   \item Wähle OD um das EW-Problem zu lösen [dadurch wird $V(\hat{x})$ einfacher]
     \begin{itemize}
       ...
       \item Eigenwertproblem von $\hat{H}$ definiert die stationäre SGL in OD:
          ...
          \end{equation}
          $\rightarrow$ homog. lin. DGL 2. Ordnung
     \end{itemize}
   ...
   \item Unterscheide zwei Klassen von Lösungen der stationären SGL:
      \begin{enumerate}
       \item gebundene Zustände
          \begin{itemize}
           \item räumlich in endlichem Gebiet lokalisiert, d.h. $ \psi(x) \overset{|x| \rightarrow \infty}{\longrightarrow} 0 $
           \item $ \hookrightarrow $ Wellenfkt. ist normierbar
       ...
       \item ungebundene Zustände
          \begin{itemize}
           ...
           \item $ \hookrightarrow $ Wellenfkt. ist \uline{nicht} normierbar und damit kein physikalisch erlaubter Zustand
          \end{itemize}
      \end{enumerate}
\end{itemize}
...
\begin{itemize}
 \item Lsg. d. stationären SGL für $ V(x) = 0 $
    ...
    \end{equation}
  \item absorbiere Vorfaktor d. kin. Energie in einer Umdef. v. E:
    \begin{equation}
  ...
  \item Alternativ: EW-Problem von $ \hat{p} $ ist schon bekannt. Lsg. f. EW-Problem von $ \hat{p} $:
    ...
    \end{equation}
  \item EW-Problem von $ \hat{p}^2 $ and $ \hat{p}^2 | p \rangle = p^2 | p \rangle \rightarrow $ zweifache Entartung des $ p^2 $ Spektrums
  ...
  \item kontinuierliches Spektrum/ungebundene Zustände:
    \begin{itemize}
     ...
     \item $ \hookrightarrow \phi_E(x) $ können nicht als physikalisch sinnvoller Systemzustand interpretiert werden
...
\begin{itemize}
  ...
  \item Betrachte Superposition von ebene-Welle-Lösungen $ \phi_E(x) $ mit Koeffizienten die durch die Fkt. $C(E) = C(k^2) $ gegeben:
    ...
    \end{equation}
    $ \hookrightarrow $ Fouriertrafo: Amplitudenfkt. C(k) ist gerade die Wellenfkt. in ID, bezeichnet mit $ \tilde{\phi}(k) $
  \item Normierbaarkeit: Wellenfkt. in ID muss quadratintegrabel \& endlich sein:
    \begin{equation}
  ...
  \item Prototyp: Gaußsches Wellenpaket:
    ...
    \end{equation}
    $ \hookrightarrow $ Fouriertrafo, also Wellenfkt. i. OD ist ebenfalls gaußförmig
    \begin{equation}
...
\begin{align}
I E&<V_0 \quad &k=\sqrt{\frac{2m}{\hbar^2}(E-V_0)}\quad &\Psi_I(x)=Ae^{kx}+A'e^{-kx}\\
II E&>0 \quad &K=\sqrt{\frac{2m}{\hbar^2}E}\quad &\Psi_{II}(x)=B\sin(kx)+B'\cos(kx)\\
III E&<V_0 \quad &k=\sqrt{\frac{2m}{\hbar^2}(E-V_0)}\quad &\Psi_{III}(x)=Ce^{kx}+C'e^{-kx}
\end{align}
Wegen Stetigkeitsbedingung von $\Psi$, $\Psi'$ und Skalierung
\begin{equation}
A'=0, \quad C=0
\end{equation}
\textbf{Paritätssymmetrie}
\begin{equation}
\hat{\Pi}|x\rangle=|-x\langle;\quad [\hat{\Pi},\hat{H}]=0
\end{equation}

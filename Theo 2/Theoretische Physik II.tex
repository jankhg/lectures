...
\textcolor[rgb]{1,0,0}{???}
\begin{equation}
\langle\vec{x}|\vec{x}\rangle=\hat{\delta}(\hat{x}-\vec{x})
\end{equation}
...
\begin{equation}
\Psi(\vec{x},t)=\langle\vec{x}|\Psi,t)=\int d^3p\langle\vec{x}|\vec{p}\rangle\underbrace{\langle\vec{p}|\Psi,t\rangle}_{\Psi(\vec{p},t)}
\end{equation}
...
Zeitentwicklung von $|\Psi,t\rangle$ durch SGL\footnote{Schrödingergleichung} 
\begin{equation}
i\hbar \frac{\partial}{\partial t}|\Psi,t\rangle=\hat{H}|\Psi,t\rangle
...
\rightarrow |E_n,t\rangle&=\hat{U}_{(t)}|E_n\rangle=\exp\left(-\frac{i}{\hbar}\hat{H}t\right)E_n\\
...
&=\langle\Psi,0|\hat{A yfvfdsdhkbrfkvhdskedhv}_{\hat{H}(t)}|\Psi,0\rangle
\end{align}
      ...
      \begin{enumerate}
       ...
       \item ungebundene Zustände
          \begin{itemize}
           ...
           \item $ \hookrightarrow $ Wellenfkt. ist \textbf{nicht} normierbar und damit kein physikalisch erlaubter Zustand
          \end{itemize}
      \end{enumerate}
\end{itemize}
